...
if( $utilObj->isUserLoggedIn()){   
	...
	$usernameOfUserEdit =   $userProfile['username']; 
	...
	if(isset($_POST['editProfileDetailsSubmit'])){  
		...
		if($captcha == $captchaSessionVar) {
			...
			$editProfileResponse =  $profileObj->editProfile(  trim($firstName), trim($lastName),  trim($dob), trim($gender), trim($address1), trim($address2), trim($city),  trim($country) , trim($contactNumber), trim($website) ,  trim($userType)  , trim($userStatus));
...
<form method="post" action="<?php echo $currentPage; ?>" id="form" >
    <table class="formFontOnly" width="53%">
        ...
        <tr>
            ...
            <td class="labelcell" ><?php echo $usernameOfUserEdit ; ?> </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <?php if($userType == $USER_TYPE_SITE_OPERATOR){ ?>
        ...
        <tr>
         ...
         <tr>
            ...
            <td class="fieldcell">
            	<select   name="userStatus" id="userStatus"  value="<?php echo htmlentities($userStatus); ?>">
                	<?php foreach ($PROFILE_STATUS_ARRAY as $statusVal){ 
							if($userStatus == $statusVal){?>
						<option selected="selected"><?php echo $statusVal ?></option>
					<?php 	}else{?>
						<option ><?php echo $statusVal ?></option>
					<?php	}?>					
					<?php }?>
