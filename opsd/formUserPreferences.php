...
if($utilObj->isUserLoggedIn()){  
	...
	$previousPreferencesArray = $profileObj->getPreferencesArray($username);
	...
	if(isset($_POST['savePreferencesSubmit'])){
		...
		if( count($newPreferencesArray) < 11){
			$developerPreferenceResponse = $profileObj->savePreferences($newPreferencesArray , $username);  
		} else{
...
<form method="post" action="projectDeveloperMyAccount" id="form" >
